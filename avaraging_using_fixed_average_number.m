...
av_n = 22; % averaging number 
q_av = 22; % main number of averaging
x_x = q_av / av_n;
q_o_s = 18; %quantity of series
sampling_frequancy = 250;
t = linspace(2, 2*(sampling_frequancy*(q_av/av_n)), sampling_frequancy*(q_av/av_n)); %time
n_s_e = [200,250,300,250,200,250]; %number of stimulus in each series of an expirement
...
main_series = zeros(sampling_frequancy*(q_av/av_n),n_o_electrodes,n_d,q_o_s);
...
for i=1:length(stimuli_1)
    for v=1:9
        if stimuli_1(i,1) == v && n_o_i(1,v) < q_av
            ...
            j = i;
            for wow =1:x_x
                if ((wow-1)*av_n) < n_o_i(1,v) && n_o_i(1,v) < (wow*av_n)+1
                    k = 1 + ((wow-1)*sampling_frequancy);
                    while k < (wow*sampling_frequancy);
                        main_series(k,:,v,1) = main_series(k,:,v,1) - stimuli_1(j,:);
                        j = j+1;
                        k = k+1;                   
                    end; 
...
n_e = 13; %number of electrode
%n_e = 10;
%n_e = 7;
sum_pz=zeros(sampling_frequancy*(q_av/av_n),18,9);
for c=1:9
    sum_pz(1:sampling_frequancy*(q_av/av_n),1:18,c) = [main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,1),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,2),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,3),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,4),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,5),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,6),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,7),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,8),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,9),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,10),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,11),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,12),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,13),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,14),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,15),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,16),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,17),main_series(1:sampling_frequancy*(q_av/av_n),n_e,c,18)];   
end;

number_of_signals = q_av/av_n;
target = zeros(250,18,1,number_of_signals);
non_target = zeros(250,18,9,number_of_signals);
...
for ii=1:18 %number of series ~ must be 18 
    ...
    if ii == 1 || ii == 4 || ii == 7 || ii == 10 || ii == 13 || ii == 16 
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,1),'y');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,2),'k');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,3),'m');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,4),'c');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,5),'b');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,6),'g');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,7),'color',[.185,.66,.245]);
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,8),'color',[.245,.158,.66]);
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,9),'r');
        for kk =1:number_of_signals
            if kk == 1 
                Start = 1;
                End = sampling_frequancy;
                target(1:250,ii,1,kk) = sum_pz(Start:End,ii,9);
                for qr=1:8
                    non_target(1:250,ii,qr,kk) = sum_pz(Start:End,ii,qr);                    
                end;
            else
                Start = 1 + sampling_frequancy*(kk - 1);
                End = sampling_frequancy*kk;
                target(1:250,ii,1,kk) = sum_pz(Start:End,ii,9);
                for qr=1:8
                    non_target(1:250,ii,qr,kk) = sum_pz(Start:End,ii,qr);                    
                end;
        ...
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,1),'r');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,2),'k');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,3),'m');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,4),'c');
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,5),'b');
        ...
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,7),'color',[.185,.66,.245]);
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,8),'color',[.245,.158,.66]);
        plot(t,sum_pz(1:sampling_frequancy*(q_av/av_n),ii,9),'y');    
        for kk =1:number_of_signals
            if kk == 1 
                Start = 1;
                End = sampling_frequancy;
                target(1:250,ii,1,kk) = sum_pz(Start:End,ii,1);
                for qr=2:9
                    non_target(1:250,ii,qr,kk) = sum_pz(Start:End,ii,qr);                    
                end;
            else
    ...
    if ii == 1 || ii == 2 || ii == 3 || ii == 13 || ii == 14 || ii == 15
        for kl = 1:number_of_signals
            [qese,zese] = findpeaks(target(126:225,ii,kl),lat);
            [bese,vese] = max(findpeaks(target(126:225,ii,kl),lat));
            p300_amp = [p300_amp bese];
            p300_lat = [p300_lat zese(1,vese)];
            for peaks=1:9
                [uese,iese] = findpeaks(non_target(126:225,ii,peaks,kl),lat);
                [oese,aese] = max(findpeaks(non_target(126:225,ii,peaks,kl),lat));
...
[Pline Pcolumn] = size(p300);
[Nline Ncolumn] = size(no_p300);
p_300_latency = ones(Pline,1);
N_300_latency = zeros(Nline,1);
result_latency = [p_300_latency;N_300_latency];
